...
import { WorldEditor } from './world_editor.js';
...
// NIEBLA
scene.fog = new THREE.FogExp2(DEFAULT_ENV.fogColor, DEFAULT_ENV.fogDensity);
...
renderer.toneMappingExposure = DEFAULT_ENV.exposure;
...
function switchLevel(levelId) {
    ...
    setTimeout(() => {
        ...
        if(worldEditor) {
             worldEditor.selectedLightIndex = -1;
             worldEditor.updateGizmoState();
        }
        ...
        loadLevel(scene, loadingManager, file, () => {
            ...
            if(worldEditor && worldEditor.visible) worldEditor.refreshLightList();
...
initUI({
    ...
    onFogChange: (value) => { 
        ...
        if(worldEditor) worldEditor.params.fogDensity = value;
    }
});
...
function updateQuestLogic(dt, time) {
    if (questState === 0) {
        if (!audioUnlocked) { 
            if (msgDisplay) { 
                msgDisplay.innerText = "Haz clic para iniciar Audio"; 
        ...
        if (checkPlatform()) { 
            ...
            if(msgDisplay) { 
                msgDisplay.style.display = 'block'; 
                msgDisplay.innerText = "ESPERA..."; 
                setTimeout(() => msgDisplay.style.display = 'none', 5000); 
            } 
            playOrbAppearSound(); 
        ...
        if (cinTime > 9.5) { 
            ...
            if(freqHint) { 
                ...
                setTimeout(() => { freqHint.style.display = 'none'; }, 4000); 
            } 
        }
        ...
        if (collectedCount === 3) {
            if(checkPlatform()) {
                if (Math.abs(playerState.speed) < 0.1) { 
                    ...
                    if(msgDisplay) { 
                        msgDisplay.innerText = "PUERTA ABIERTA"; 
                        msgDisplay.style.display = 'block'; 
                    } 
                } else { 
                    if(msgDisplay) { 
                        msgDisplay.innerText = "QUIETO EN EL ALTAR"; 
                        msgDisplay.style.display = 'block'; 
                    } 
                } 
            } else { 
                if(msgDisplay) { 
                    msgDisplay.innerText = "VUELVE AL ALTAR"; 
                    msgDisplay.style.display = 'block'; 
                } 
            }
        } else { 
        ...
        if (playerState.container.position.z > levelState.doorsCenter.z + 2.0) switchLevel(2);
    }
}
